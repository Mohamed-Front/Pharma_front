  ...
  <va-card class="card my-4 p-[2%] min-h-[50vh] max-w-7xl m-auto">
    <div class="flex">
      <div>
        ...
        <div v-else-if="currentTermsAndConditions" class="prose max-w-none" :dir="appLang === 'ar' ? 'rtl' : 'ltr'">
          <div v-html="sanitizedTermsAndConditions" />
        </div>
...
/* Reactive data                                                      */
/* ------------------------------------------------------------------ */
const termsAndConditionsEn = ref('');
const termsAndConditionsAr = ref('');
...
const currentTermsAndConditions = computed(() => {
  return appLang.value === 'ar' ? termsAndConditionsAr.value : termsAndConditionsEn.value;
});
...
onMounted(async () => {
  ...
  try {
    ...
    termsAndConditionsEn.value = payload.terms_and_conditions_en ?? '';
    termsAndConditionsAr.value = payload.terms_and_conditions_ar ?? '';
    ...
    error.value = e.response?.data?.message || t('error.loadTermsAndConditions');
    toast.add({
      severity: 'error',
