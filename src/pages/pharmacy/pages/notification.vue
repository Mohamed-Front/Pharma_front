...
// Initial state for notification categories
...
// --- Computed for active data ---
const activeData = computed(() => {
  return activeTabIndex.value === 0 ? adminNotificationsData.value : warehouseNotificationsData.value
})

const activeTotal = computed(() => activeData.value.total)
...
const updateCurrentList = () => {
  currentNotifications.value = activeData.value.data
  currentPage.value = activeData.value.current_page
}
...
const markAllAsRead = async () => {
  loading.value = true
  try {
    const type = activeTabIndex.value === 0 ? 'admin' : 'warehouse'
    await axios.post(`/api/notification/read-all?type=${type}`)

    const targetData = activeTabIndex.value === 0 ? adminNotificationsData : warehouseNotificationsData
    targetData.value.data.forEach(n => {
...
const viewNotification = (notification) => {
  ...
  if (notification.type === 'warehouse') {
    if (notification.redirect_type === 'order_status_changed' && metadata.order_id) {
      router.push({
        name: 'pharmacy-orders',
      })
    } else if (notification.redirect_type === 'offer_created' && metadata.offer_id) {
      router.push({
        name: 'pharmacy-offers',
      })
    } else {
      console.log(`Unhandled warehouse redirect: ${notification.redirect_type}`)
    }
  } else if (notification.type === 'admin') {
    console.log(`Admin notification clicked: ${notification.title}`)
    // Add admin routing logic here if needed
  }
}
...
// Watch tab change to update list
  ...
  <div class="min-h-screen bg-gray-100 flex justify-center items-start p-4">
    <div class="w-full max-w-7xl bg-white rounded-2xl shadow-xl overflow-hidden">
        ...
        <TabPanel>
          <template #header>
            ...
            >
              ...
              <span>{{ t('notifications.warehouse_notes') || 'Warehouse Notes' }}</span>
              <Badge
      ...
      <div
        ...
        :id="activeTabIndex === 0 ? 'admin-notifications' : 'warehouse-notifications'"
        role="tabpanel"
      >
        <!-- Mark All As Read Button -->
        <div
          v-if="activeTotal > 0 && !loading && !error"
        ...
        >
          <Button
            ...
            outlined
            @click="markAllAsRead"
            :disabled="activeData.unread_count === 0"
            class="text-sm"
          />
        </div>
